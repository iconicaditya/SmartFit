...
fun ActivityFormRoute(
    ...
    val factory = remember(appContainer) {
        object : ViewModelProvider.Factory {
            override fun <T : ViewModel> create(modelClass: Class<T>): T {
                if (modelClass.isAssignableFrom(ActivityFormViewModel::class.java)) {
                    @Suppress("UNCHECKED_CAST")
                    return ActivityFormViewModel(appContainer.activityRepository) as T
                }
    ...
    val formViewModel: ActivityFormViewModel = viewModel(factory = factory)
    val uiState by formViewModel.uiState.collectAsState()

    LaunchedEffect(activityId) {
        formViewModel.load(activityId)
    }
    ...
    ActivityFormScreen(
        ...
        onEvent = { event ->
            when (event) {
                ActivityFormUiEvent.OnSubmitClicked -> formViewModel.onEvent(event, onBackClick)
